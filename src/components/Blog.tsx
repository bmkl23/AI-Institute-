...
import { useBlogs, Blog as BlogType } from "../hooks/useBlogs";
...
/* ─── MODAL ─── */
function BlogModal({ blog, onClose }: { blog: BlogType; onClose: () => void }) {
  return (
    <AnimatePresence>
      <motion.div
        initial={{ opacity: 0 }}
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col gap-2 p-5">
        <h3 className="text-[15px] font-bold leading-snug text-white transition group-hover:text-blue-300">
          {title}
        </h3>
        <p className="text-[12.5px] leading-relaxed text-slate-400 line-clamp-3">
          {description}
        </p>
...
export function Blog() {
  ...
  return (
    <>
      ...
      >
        <div className="pointer-events-none absolute inset-0 bg-gradient-to-b from-blue-950/10 via-transparent to-transparent" />
            ...
            <div className="flex items-center justify-center py-20">
              <div className="flex gap-2">
                {[0, 1, 2].map((i) => (
                  <div
                    key={i}
                    className="h-2 w-2 animate-bounce rounded-full bg-blue-400/60"
